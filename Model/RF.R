...
urlfile="https://raw.githubusercontent.com/DATA-MINING23-BaseBall/R/main/Data_pre/basic_df_sample.csv"
df<-read_csv(url(urlfile))
head(df)
DF = df[-1]    # basic_df_sample.csv, df5_sample.csv, lFinal_df_sample.csv
head(DF)
...
get_rf_result = function(iter_num, cv_num, df, n, train_n, ntree_para, mtry_para){
...
# df별 전체 표 생성
ntrees = c(100, 200, 300, 400, 500, 600, 700, 800, 900, 1000)
step = ncol(DF)%/%6
df.tbl = get_rf_result(100, 5, DF, n, train_n, c(250, 500, 750, 1000), seq(step, step*5, step)); df.tbl
df.tbl = get_rf_result(10, 1, DF, n, train_n, ntrees, c(ncol(DF)%/%3)); df.tbl
