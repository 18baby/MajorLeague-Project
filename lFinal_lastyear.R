...
## SJ :: year-1 문제
pfinal.xxx = pfinal_pitcher_Team.df[, c('playerID', 'yearID', 'teamID')]
mysalaries_xxx = my_salaries[, c('playerID', 'yearID', 'teamID', 'salary')]
year_init = data.frame(yearID = seq(from=1997, to=2016))
## year_init를 선수로 그룹핑 한거에 조인.
library(sqldf)
mysalaries_y = sqldf("select 
         playerID, 
         year_init.yearID, 
         teamID, 
         sum((mysalaries_xxx.yearID = year_init.yearID) * salary) salary 
       from mysalaries_xxx 
       join year_init
       group by 1,2")
mysalaries_y$yearID.y = mysalaries_y$yearID-1
mysalaries_y[mysalaries_y$salary==0, 'salary'] = NA

mysalaries_y = mysalaries_y %>% arrange(playerID, yearID) %>% group_by(playerID) %>%  fill(salary, .direction = "down")
mysalaries_y = subset(mysalaries_y, select=-yearID)
mysalaries_y = mysalaries_y %>% rename("yearID" = "yearID.y")


lFinal.df = left_join( pfinal_pitcher_Team.df, mysalaries_y, by=c('playerID', 'yearID'), multiple = "all")    # 1998년~2022년 모든 투수 데이터
lFinal.df = lFinal.df %>% rename("teamID" = "teamID.x")
lFinal_pre = lFinal.df[, c('playerID', 'yearID', 'teamID', 'salary')]
lFinal.df = lFinal.df %>% filter(!is.na(salary))   # 채울 수 있는 값 채우기
lFinal.df = lFinal.df[-69]
colSums(is.na(lFinal.df))
## SJ :: salary가 0인 값은 삭제, NA 값은 위에서부터 추가 
