...
summary(lFinal.df)
summary(iFinal.df)
lFinal.df[is.na(lFinal.df$salary), ]   # salary값이 없는 데이터 9,616개 -> 채울 방법??
...
colSums(is.na(iFinal.df))
  ...
  # (1) 리그 ERA Inf값 처리
iFinal.df[iFinal.df$l_ERA == Inf, c("playerID", "yearID", "l_ER", "l_IPouts")]   # 6739번
iFinal.df[is.na(iFinal.df$l_ERA), c("playerID", "yearID", "l_ER", "l_IPouts")]   # 2520, 5325번
iFinal.df[6739, "l_ERA"] = 200    # 무한대값 -> 200으로 채우기

  # (2) NA값 처리 -> 3분위수 5.325로 채우기
    # [1] 평균차책 -> 특별 처리
third = quantile(iFinal.df$l_ERA, 0.75, na.rm=T)  # 3분위수
iFinal.df[is.na(iFinal.df$l_ERA), "l_ERA"] = third
...
for( col in col_names ){
  iFinal.df[[col]][is.na(iFinal.df[[col]])] = 0    # 열별 확인
}
colSums(is.na(iFinal.df))
      # po_에서 Inf값인 선수 처리
iFinal.df[iFinal.df$po_ERA == Inf, c("playerID", "yearID", "po_ER", "po_IPouts")]
iFinal.df[iFinal.df$po_ERA == Inf, "po_ERA"] = 200

  # (3) 라벨인코딩
col_names = c("teamID", "lgID", "birthCountry", "throws", "divID")
for( col in col_names ){
    # 라벨인코딩
  datas = unique(iFinal.df[[col]])
  iFinal.df[[col]] = as.numeric(factor(iFinal.df[[col]], levels = datas))
}

  # (4) debut 날짜만 저장
years = sapply(iFinal.df$debut, function(x) format(as.Date(x), "%Y"))
iFinal.df$debut = as.numeric(years)
